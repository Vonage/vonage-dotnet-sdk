...
namespace Vonage.Test.Unit.Meetings.GetDialNumbers
{
    public class GetDialNumbersTest
    {
        private Func<Task<Result<GetDialNumbersResponse[]>>> Operation =>
            () => this.client.GetDialNumbersAsync(this.request);

        private readonly GetDialNumbersRequest request = new GetDialNumbersRequest();
        ...
        private IRequestBuilder CreateRequest() =>
            WireMockExtensions
                .CreateRequest(this.helper.Token, this.request.GetEndpointPath())
                .UsingGet();
    }
}